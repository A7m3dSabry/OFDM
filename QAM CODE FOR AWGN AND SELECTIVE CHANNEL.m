...
function main(input_signal,channel,h,noise_start,noise_end,figureTitle)
	BER = [];
	BER = [BER ; ofdm(input_signal,@(a) mapper_16qam(a), 1,...
	16,16,...
	2.5,channel ,h,noise_start,noise_end,...
	[-2 0 2].',[-3 -1 1 3].',[1 0].')];
	BER = [BER ; ofdm(input_signal,@(a) mapper_16qam(a), 3,...
	16,16,...
	2.5, channel ,h,noise_start,noise_end,...
	[-2 0 2].',[-3 -1 1 3].',[1 0].')];
...
function [outputSignal] = mapper_16qam(input_signal)
	...
	outputSignal =toRow( sum(outputSignal(:,[1 3]) .* outputSignal(:,[2 4]),2)) ;%%check if;
end
%% channels
